...
func (s *Service) computeStateUpdate(userID int64, resp *SpotifyTrackResponse) stateAction {
	...
	if state.accumulatedMs >= track.DurationMs {
		...
		s.logger.Printf("Song repeat detected for user %d: %s", userID, track.Name)
	}
...
func (s *Service) fetchTrackForUser(ctx context.Context, userID int64) {
	// Fetch from Spotify (no lock held)
	...
	// Compute state changes (holds lock internally via defer)
	...
	if action.stampTrack {
		s.stampTrack(ctx, userID, action.track, action.accumulatedMs)
	}
}
...
func (s *Service) stampTrack(ctx context.Context, userID int64, track *models.Track, accumulatedMs int64) {
	s.logger.Printf("User %d stamped track: %s by %s (acc: %dms)", userID, track.Name, getFirstArtist(track), accumulatedMs)
